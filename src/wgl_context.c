                                ...
                                const _GLFWfbconfig* fbconfig)
{
    ...
    for (i = 0;  i < nativeCount;  i++)
    {
        ...
        if (_glfw.wgl.ARB_pixel_format)
        {
            ...
            if (FIND_ATTRIB_VALUE(WGL_PIXEL_TYPE_ARB) != WGL_TYPE_RGBA_ARB && !fbconfig->floatbuffers)
                continue;

            if (FIND_ATTRIB_VALUE(WGL_PIXEL_TYPE_ARB) != WGL_TYPE_RGBA_FLOAT_ARB && fbconfig->floatbuffers)
                continue;
            ...
            u->floatbuffers = (FIND_ATTRIB_VALUE(WGL_PIXEL_TYPE_ARB) == WGL_TYPE_RGBA_FLOAT_ARB);
